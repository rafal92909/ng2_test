...
mcafee_wa_popup.prototype.showSiteReport = function (tab)
{
    if (tab.url != null && tab.url.length > 0)
    {
        ...
        var properties = {};
        properties.url = bkPage.mcafee_wa_bkconstants.siteReportUrl + encodedURI;
...
mcafee_wa_popup.prototype.showOptions = function (tab)
{
    if (tab.url != null && tab.url.length > 0)
    {
        var bkPage = chrome.extension.getBackgroundPage();
        var encodedURI = encodeURI(tab.url);
        bkPage.mcafee_wa_bkglobals.messageDispatcher.onSettings(encodedURI);
    }
};
...
mcafee_wa_popup.prototype.init = function ()
{
    ...
    document.addEventListener('DOMContentLoaded',  function () 
    {
        chrome.tabs.query({'active' : true, 'currentWindow' : true }, function (tabs)
         {
             var bkPage=  chrome.extension.getBackgroundPage();
             var resourcerequestor = new bkPage.mcafee_wa_resourcerequestor();
